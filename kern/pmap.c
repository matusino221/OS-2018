...
/*
Naprogramuj jednoduchy alokátor fyzickej pamete.
ak n > 0 alokuj n - stranok bitoch . Neinicializuj pamet! Vrat virtualnu adresu kernelu
 
ak n == 0 vrat next free stranku
 
ak si mimo pameta pouzi panic
...
boot_alloc(uint32_t n)
{
	...
	/*
	Alokuj n ramcekov z pamete a potom aktualizuj nextfree
...
mem_init(void)
{
	...
	// Your code goes here:
	/*
	Alokuj npages z struktury PageInfo a uloz ju do pages
	pouzi memset na inicializaciu prvkov pola struktury Pageinfo a nastav ich na 0
	*/
...
page_init(void)
{
	...
	char *va;
	/*
	Sprav funkciu ktora inicializuje stranku zo struktury PageInfo 
	1) Ak je fyzicka pamet 0 nepouziva sa
	2) Od PGSIZE po npages*PGSIZE je volna // pre nas to znamena ze sa moze alokovat
	3) Od IOPHYSMEM po EXTPHYSMEM nesmie byt nikdy alokovana !
	*/
	//prechadzam vsetky stranky vo strukture 
	for (i = 0; i < npages; i++) {
		pa = page2pa(&pages[i]); //prevediem si stranku na fyzicku adresu
		va = page2kva(&pages[i]); // prevediem si stranku na adresu kernelu 
		if ( (i==0) || ( IOPHYSMEM <= pa + PGSIZE && va < (char *) boot_alloc(0) ) ){ // i=0 je pripad 1)  a druhy pripad je diera od IOPHYSMEM  po adresu jadra ( ako zistim adresu jadra ? boot_alloc(0) ) samozrejme musim porovnavat rovnake typy
			pages[i].pp_ref = 1;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!(*pde & PTE_P)){
		...
		*pde = page2pa(pp) | PTE_W | PTE_P | PTE_U;
		...
		pte = (pte_t *) KADDR(page2pa((struct PageInfo*) pp));
	}
	else {
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	//ak neexistuje tak vratim E_NO_MEM
	if (!pte){
		return -E_NO_MEM;
	}
	tlb_invalidate(pgdir, va);
	//pridam pocet stranok
	pp->pp_ref += 1;
	//vymaze ak tam uz nieco je
	if(*pte & PTE_P){
		page_remove(pgdir, va);
	}
	// nastavim perm a PTE_P
	*pte = pa | perm | PTE_P;
